...
<body>
    <div class="header">
        <h2 style="margin-bottom: 4px; margin-top: 0;">LAPORAN LEMBUR KARYAWAN</h2>
        @php
            function parseCarbon($value, $is_translate = false)
            {
                if ($is_translate) {
                    $data = \Carbon\Carbon::parse($value)->locale('id')->translatedFormat('d M Y H:i:s');
                } else {
                    $data = \Carbon\Carbon::parse($value)->format('d M Y H:i:s');
                }
                return $data;
            }
        ...
        <tbody>
            @forelse ($report as $index => $data)
                <tr class="nowrap">
                    ...
                    <td>{{ parseCarbon($data->start_time, true) }}</td>
                    <td>{{ parseCarbon($data->end_time, true) }}</td>
                    <td>{{ $data->approved ? 'Disetujui' : 'Belum Disetujui' }}</td>
